...
with st.form("Form 1"): 
    ...
    if submit:
        ...
        with tab1:
            st.write("Bản mã hóa : " )
            st.code(Hoan_vi.encrypt(text= text, key= key)) 
        with tab2:
            st.write("Bản giải mã : " )
            st.code(Hoan_vi.decrypt(text= text,key= key)) 
    ...
    if submit:
        ...
        if st.session_state["options"] == "Encrypt":
            if file is not None:
                ...
                content = Hoan_vi.encrypt(text= text,key= key)
        if st.session_state["options"] == "Decrypt":
            if file is not None:
                content_decrypted = file.read().decode("utf-8")
                content = Hoan_vi.decrypt(text= text,key= key)
